...
const SimplifyForm = () => {
  ...
  useEffect(() => {
    // Combine country and state fetching logic into a single effect
    const fetchData = async () => {
      try {
        // Fetch countries
        const countriesResponse = await axios.get(apiEndpoints.Country);
        const countriesData = countriesResponse.data.Data || [];
        setCountries(countriesData);

        // Set India as default if no country selected
        if (!selectedCountry && countriesData.length) {
          const india = countriesData.find(country => country.Name === "India");
          if (india) {
        ...
        if (selectedCountry) {
          const statesResponse = await axios.get(apiEndpoints.State, {
    ...
    // Update ShowContact whenever country changes
    setShowContact(selectedCountry === "123");

  }, [selectedCountry]);

  /**
   * Handles the form submission for creating a new Saniiro account.
   * @param {object} values - Form values.
   * @param {object} actions - Formik actions (e.g., resetForm).
   */
  const handleSubmit = async (values, { resetForm }) => {
    setLoading(true);
    try {
      const payload = { ...values, OtpId: otpId };
      ...
      if (response?.data?.Status === 1000) {
        ...
        SweetAlert.error("Error", response?.data?.message || "Account creation failed. Please try again.");
      }
    } catch (error) {
